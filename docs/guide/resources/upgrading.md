...
## `@wxt-dev/storage` v1.x &rarr; v2.0

The `@wxt-dev/storage` package is versioned independently from `wxt`. If you install it directly (or via `wxt/storage` re-export) and were using caller-invented value generics or the object-keyed `migrations` form, this section applies.

The design principle: **every unverified compile-time type assertion in the public API has been replaced by either forced narrowing (`unknown`) or verified narrowing (schema).** The old `getItem<T>(key)` signature let callers assert `T` with zero evidence — the type parameter appeared only in return position, so TypeScript blindly trusted it. The new signatures make trust boundaries explicit.

### Value generics removed on `getItem`, `getMeta`, `setItem`, `setMeta`, `watch`

All five methods previously accepted a `<T>` type parameter. They no longer do. The return type is `unknown` (or `Record<string, unknown>` for metadata), and callers must narrow.

**Preferred migration: use `defineItem` with a schema.** The type is bound at definition and validated at runtime.

```ts
// [!code --]
const user: User | null = await storage.getItem<User>('local:user');

// [!code ++]
const userItem = storage.defineItem('local:user', {
  schema: z.object({ id: z.string(), name: z.string() }),
  fallback: null,
});
const user = await userItem.getValue(); // User | null, validated on read
```

**Call-site migration: narrow `unknown`.**

```ts
// [!code --]
const v = await storage.getItem<User>('local:user');

// [!code ++]
const raw = await storage.getItem('local:user');
const v = isUser(raw) ? raw : null;
```
...
```ts
// [!code --]
const meta = await storage.getMeta<{ v: number }>('local:count');

// [!code ++]
const meta = (await storage.getMeta('local:count')) as { v: number };
```

**`watch` callback.** Callback params are now `(newValue: unknown, oldValue: unknown)` instead of `(newValue: T | null, oldValue: T | null)`. The runtime behavior (including `null` on remove) is unchanged — only the compile-time nullability nudge is gone.

```ts
// [!code --]
storage.watch<User>('local:user', (newVal, oldVal) => {
  if (newVal) console.log(newVal.name);
});

// [!code ++]
storage.watch('local:user', (newVal, oldVal) => {
  const user = isUser(newVal) ? newVal : null;
  if (user) console.log(user.name);
});
```

**`setItem` / `setMeta`.** The value/properties params were re-typed as `unknown` and `Record<string, unknown> | null` respectively. Users of the caller-invented generic form must drop the type argument; the write itself accepts the same values.

```ts
// [!code --]
await storage.setItem<User>('local:user', user);
await storage.setMeta<{ v: number }>('local:count', { v: 3 });

// [!code ++]
await storage.setItem('local:user', user);
await storage.setMeta('local:count', { v: 3 });
```

### `restoreSnapshot` data parameter typed

```ts
// [!code --]
await storage.restoreSnapshot('local', JSON.parse(text));
...
### Migrations changed from object form to positional tuple

The old `migrations: Record<number, fn>` form supported arbitrary numeric keys. The new `migrations: ReadonlyArray<fn>` form is ordered by target version: position `i` (0-indexed) migrates version `i+1` → `i+2`. Non-contiguous version numbers are no longer supported.
...
storage.defineItem<number>('local:count', {
  version: 3,
  migrations: [
    (v) => Number(v), // position 0: v1 → v2
    (v: number) => v + 1, // position 1: v2 → v3
  ],
});
```

For chain-typed migrations where each function's return type is verified against the next function's parameter, use `defineMigrations<TValue>()`:

```ts
import { defineMigrations } from '@wxt-dev/storage';

storage.defineItem<number>('local:count', {
  version: 3,
  migrations: defineMigrations<number>()
    .add((v: unknown) => Number(v))
    .add((v: number) => v + 1)
    .build(),
});
```

### `getItems` return type narrowed

Previously `Array<{ key: StorageItemKey; value: any }>`; now per-slot narrowed via `GetItemsResult<T>`. Bare literal keys yield `{ key: <literal>; value: unknown }`; `WxtStorageItem` entries yield the item's `TValue`.

```ts
// [!code --]
const results = await storage.getItems(['local:count', 'local:name']);
results[0].value.someProp; // was `any`, silently allowed

// [!code ++]
const results = await storage.getItems(['local:count', 'local:name']);
results[0].value.someProp; // Error: 'someProp' does not exist on type 'unknown'
// Migrate to defineItem for typed retrieval:
const count = storage.defineItem<number>('local:count', { fallback: 0 });
const name = storage.defineItem<string>('local:name', { fallback: '' });
const [c, n] = await storage.getItems([count, name]);
c.value; // number
n.value; // string
```
