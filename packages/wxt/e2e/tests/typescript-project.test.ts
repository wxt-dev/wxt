...
describe('TypeScript Project', () => {
  ...
  it('should augment the types for browser.scripting.executeScript', async () => {
    const project = new TestProject();
    project.addFile(
      'entrypoints/script.tsx',
      'export default defineUnlistedScript(() => {});',
    );

    await project.prepare();

    const output = await project.serializeFile('.wxt/types/browser.d.ts');
    expect(output).toMatchInlineSnapshot(`
      ".wxt/types/browser.d.ts
      ----------------------------------------
      // Generated by wxt
      import "wxt/browser";
      ...
      declare module "wxt/browser" {
        type TypedScriptInjection = Parameters<
            WxtBrowser["scripting"]["executeScript"]
          >[0] & { files: Array<"/script.js">; func?: never | undefined };

          type ReplaceFirstArg<Fnc, Arg> = Fnc extends {
            <Args extends any[], Result>(injection: any): infer U;
            <Args extends any[], Result>(injection: any, callback: infer C): infer V;
          }
            ? {
                <Args extends any[], Result>(injection: Arg): U;

                <Args extends any[], Result>(injection: Arg, callback: C): V;
              }
            : never;

        interface WxtTypedBrowser {
          scripting: {
            executeScript: ReplaceFirstArg<
              WxtBrowser["scripting"]["executeScript"],
              TypedScriptInjection
            >;
          };
        }

        declare const browser: WxtTypedBrowser & WxtBrowser;
      }
      "
    `);
  });
