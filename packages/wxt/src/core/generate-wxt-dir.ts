...
export async function generateWxtDir(entrypoints: Entrypoint[]): Promise<void> {
  ...
  entries.push(getBrowserEntry(entrypoints));
...
function getBrowserEntry(entrypoints: Entrypoint[]): WxtDirFileEntry {
  const paths = entrypoints
    .filter((entrypoint) =>
      isEntryPointOfType(entrypoint, ['unlisted-script', 'content-script']),
    )
    .map((entrypoint) =>
