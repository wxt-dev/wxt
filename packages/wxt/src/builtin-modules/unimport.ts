...
export default defineWxtModule({
  ...
  setup(wxt) {
    ...
    wxt.hooks.hook('ready', () => {
      ...
      wxt.unimport = createUnimport(options);
    });

    // Generate types
    wxt.hooks.hook('prepare:types', async (wxt, entries) => {
      entries.push(await getImportsDeclarationEntry(wxt));

      if (!options.eslintrc.enabled) return;
      entries.push(await getImportsEslintEntry(wxt.unimport, options));
    });

    // Add vite plugin
    addViteConfig(wxt, () => ({
      plugins: [vitePlugin(wxt.unimport)],
    }));
  },
});
...
async function getImportsDeclarationEntry(wxt: Wxt): Promise<WxtDirFileEntry> {
  // Load project imports into unimport memory so they are output via generateTypeDeclarations
  await wxt.unimport.scanImportsFromDir(undefined, { cwd: wxt.config.srcDir });

  return {
    path: 'types/imports.d.ts',
    text: [
      '// Generated by wxt',
      await wxt.unimport.generateTypeDeclarations(),
      '',
    ].join('\n'),
    tsReference: true,
  };
